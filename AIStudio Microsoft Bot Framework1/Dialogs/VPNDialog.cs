        ...
        private async Task receivedProblem(IDialogContext context, IAwaitable<string> result)
        {
            try
            {
                ...
                switch (message[0])
                {
                    ...
                    case '5':
                        //"5. I can't access internal website"
                        await context.PostAsync("If you are receiving a SSL certificate error, it means you have not installed your certificate properly. Please download and install the certificate from our external website.");
                        break;
                    case '6':
